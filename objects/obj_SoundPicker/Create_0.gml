...
function trigger() {
	...
	array_foreach(atarray, function(_v, _i) {
		var _pos = string_pos(".ogg", _v);
		if _pos != 0 {
			var _n = string_copy(_v, 1, _pos - 1)
			atarray[_i] = string("sound_get(\"{0}\")", _n);
		}
	});
	
	array_foreach(all_sounds_builtin, function(_v, _i) {
		array_push(atarray, string("asset_get(\"{0}\")", _v));
	});
}
...
function click_event(_selected) {
	...
	obj_EDITORattacks.selection_end("sound");
