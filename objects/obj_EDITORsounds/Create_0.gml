...
all_sounds_custom = get_custom_sounds(string("{0}/sounds/", global.selected_path));

draw_set_halign(fa_left);


function lookfor_uses_custom() {
	// save formatt: ["file", appearances, extra data]
	
	var _path = string("{0}/scripts/", global.selected_path)
	var _allfiles = get_all_files(_path);
	var _finalized = [];
	
	for (var i = 0; i < array_length(_allfiles); i++) {
		if (filename_ext(_allfiles[i]) == ".gml") {
			var _count = count_string_in_file(_path + _allfiles[i], "sound_get(\"" + selected + "\")");
			if _count > 0 {
				array_push(_finalized, [_allfiles[i], _count, 0]);
			}
		}
	}
	return _finalized;
...
function lookfor_uses_builtin() {
	...
	for (var i = 0; i < array_length(_allfiles); i++) {
		if (filename_ext(_allfiles[i]) == ".gml") {
			var _count = count_string_in_file(_path + _allfiles[i], "asset_get(\"" + selected + "\")");
			if _count > 0 {
				array_push(_finalized, [_allfiles[i], _count, 0]);
			}
		}
...
function count_string_in_file(_file_path, _target_string) {
	if (!file_exists(_file_path)) return 0;
	
	var _file = file_text_open_read(_file_path);
	var _count = 0;
	
	while (!file_text_eof(_file)) {
		_count += string_count(_target_string, file_text_read_string(_file));
...
function swap_and_shit(_path, _stringtocheck1, _stringtocheck2, _fullstring) {
	if !file_replace_line_starting_with(_path, _stringtocheck1, _fullstring) {
		if !file_replace_line_starting_with(_path, _stringtocheck2, _fullstring) {
			if !file_append_line(_path, _fullstring) {
				show_message("Missing init.gml file!");
			}
		}
	}
}

function update_display(_checkvar, _layertextid, _newtext) {
	switch _checkvar {
		case 2:
			layer_text_text(_layertextid, _newtext);
			layer_text_blend(_layertextid, c_lime);
			break;
		case 1:
			layer_text_text(_layertextid, _newtext);
			layer_text_blend(_layertextid, c_orange);
			break;
		case 0:
			layer_text_text(_layertextid, "NO SOUND");
			layer_text_blend(_layertextid, #992D2D);
			break;
	}
}

function populate_attin(_path, _varnametheme, _varnametype, _texttocheck, _addbracket) {
	ATTIN[$ _varnametheme] = file_extract_string_between_char(_path, _texttocheck, "\"");
	var _functype = file_extract_string_between_char(_path, _texttocheck, "(", _addbracket);
	if _functype == "sound_get" {
		ATTIN[$ _varnametype] = 2;
	} else if _functype == "asset_get" {
		ATTIN[$ _varnametype] = 1;
	} else {
		ATTIN[$ _varnametype] = 0;
	}
}

function checkbutton(_x, _y, _soundget, _assetget, _replacestring, _path, _function) {
	if mouse_in_rectangle(_x, _y, _x + 240, _y + 64) {
		set_cursor(cr_handpoint);
		if mouse_check_button_pressed(mb_left) {
			swap_and_shit(_path, _soundget, _assetget, _replacestring)
...
function GET_ME_WHAT_I_NEED() {
	var _path = string("{0}/scripts/init.gml", global.selected_path)
	var _path2 = string("{0}/scripts/load.gml", global.selected_path)
	

	ATTIN = {} // All The Things I Need
	
	populate_attin(_path, "victorytheme", "victorytype", "set_victory_theme", false);
	if ATTIN.victorytype == 0 {
		populate_attin(_path2, "victorytheme", "victorytype", "set_victory_theme", false);
	}
	populate_attin(_path, "landtheme", "landtype", "land_sound = ", true);
	populate_attin(_path, "landlagtheme", "landlagtype", "landing_lag_sound = ", true);
	populate_attin(_path, "wavelandtheme", "wavelandtype", "waveland_sound = ", true);
	populate_attin(_path, "jumptheme", "jumptype", "jump_sound = ", true);
	populate_attin(_path, "djumptheme", "djumptype", "djump_sound = ", true);
	populate_attin(_path, "airdodgetheme", "airdodgetype", "air_dodge_sound = ", true);
